...
import { Briefcase, Filter, Languages, Megaphone, Plus, ReceiptText, Search, Star, Trash2 } from "lucide-react";
...
export default function Home() {
  ...
  async function loadMine() {
    ...
    try {
      ...
      setToast({ type: "error", message: error instanceof Error ? error.message : labels.error });
    }
  }
  ...
  async function requestUpgrade(formData: FormData) {
    ...
    try {
      ...
      setToast({ type: "success", message: `${labels.success}: ${upgradeAmounts[upgradeType]} ETB` });
    } catch (error) {
      setToast({ type: "error", message: error instanceof Error ? error.message : labels.error });
    } finally {
      setBusy(false);
  ...
  if (loading) {
    return <main className="mobile-shell grid place-items-center p-6 text-sm text-ink">{labels.loading}</main>;
  }
  ...
  return (
    <main className="mobile-shell pb-24">
      ...
      {view === "post" && <PostForm busy={busy} language={language} onSubmit={createListing} />}
      {view === "my-ads" && (
        <MyAds
          busy={busy}
          listings={myListings}
      ...
      <nav className="fixed bottom-0 left-1/2 z-30 grid w-full max-w-[480px] -translate-x-1/2 grid-cols-4 border-t border-black/10 bg-white p-2">
        ...
        <NavButton
          ...
          disabled={!session?.profile.is_admin}
        />
      </nav>

      {selected && <ListingSheet listing={selected} language={language} onClose={() => setSelected(null)} />}
      {toast && <ToastView toast={toast} onClose={() => setToast(null)} />}
      {busy && <div className="fixed inset-0 z-40 grid place-items-center bg-white/50 text-sm font-bold">{labels.loading}</div>}
    </main>
  );
}
...
function TabButton({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return (
    <button className={`h-11 rounded-md text-sm font-bold ${active ? "bg-leaf text-white" : "text-ink/70"}`} onClick={onClick}>
      {label}
    </button>
  );
}
...
function ListingGrid({ listings, language, onSelect }: { listings: Listing[]; language: Language; onSelect: (listing: Listing) => void }) {
  if (listings.length === 0) return <p className="py-12 text-center text-sm text-ink/60">{t(language, "empty")}</p>;
  return (
        ...
        >
          <div className="flex gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {listing.category && <p className="mt-1 text-xs font-bold text-ink/45">{categoryLabel(listing.type, listing.category, language)}</p>}
            </div>
          </div>
        </button>
      ))}
...
function ListingSheet({ listing, language, onClose }: { listing: Listing; language: Language; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end bg-black/35" onClick={onClose}>
      <section className="max-h-[88dvh] w-full max-w-[480px] overflow-y-auto rounded-t-2xl bg-white p-4 shadow-sheet" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="flex gap-2 overflow-x-auto">
          {(listing.listing_images?.length ? listing.listing_images : [{ id: "blank", public_url: "" } as any]).map((image) => (
            <div key={image.id} className="grid h-52 min-w-full place-items-center overflow-hidden rounded-lg bg-mist">
              {image.public_url ? <img className="h-full w-full object-cover" src={image.public_url} alt="" /> : <Megaphone className="text-ink/35" />}
            </div>
          ))}
        </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3">
          <a className="grid h-12 place-items-center rounded-lg bg-leaf font-black text-white" href={`tel:${listing.phone}`}>{t(language, "call")}</a>
          <a className="grid h-12 place-items-center rounded-lg bg-ember font-black text-white" href={username ? `https://t.me/${username}` : "#"}>{t(language, "telegram")}</a>
        </div>
      </section>
    </div>
  );
...
function PostForm({ busy, language, onSubmit }: { busy: boolean; language: Language; onSubmit: (formData: FormData) => void }) {
  ...
  return (
    ...
    >
      ...
      <Field name="title" placeholder={type === "item" ? formCopy.itemTitle : formCopy.jobTitle} required />
      ...
      {type === "item" ? (
        <>
          <Field name="price" placeholder={formCopy.price} type="number" required />
      ...
      <textarea className="min-h-28 w-full rounded-lg border border-black/10 bg-white p-3 outline-none" name="description" placeholder={type === "job" ? formCopy.requirements : formCopy.details} />
      <button className="h-12 w-full rounded-lg bg-leaf font-black text-white disabled:opacity-60" disabled={busy}>{t(language, "submit")}</button>
    </form>
  );
}

function Field({ name, placeholder, type = "text", required }: { name: string; placeholder: string; type?: string; required?: boolean }) {
  return <input className="h-12 w-full rounded-lg border border-black/10 bg-white px-3 outline-none" name={name} placeholder={placeholder} type={type} required={required} />;
}

function SelectField({ children, label, name, required }: { children: ReactNode; label: string; name: string; required?: boolean }) {
  return (
    <select className="h-12 w-full rounded-lg border border-black/10 bg-white px-3 outline-none" name={name} aria-label={label} required={required}>
      {children}
    </select>
  );
}
...
function MyAds({ busy, listings, language, onDelete, onRequestUpgrade }: { busy: boolean; listings: Listing[]; language: Language; onDelete: (listing: Listing) => void; onRequestUpgrade: (formData: FormData) => void }) {
  return (
    <section className="space-y-4 p-4">
      <form
      ...
      >
        ...
        <select className="h-11 w-full rounded-md border border-black/10 px-3" name="upgradeType">
        ...
        <select className="h-11 w-full rounded-md border border-black/10 px-3" name="listingId">
        ...
        <div className="rounded-md bg-mist p-3 text-xs leading-5">
          Telebirr: {appConfig.payment.telebirrName} / {appConfig.payment.telebirrNumber}<br />
        ...
        <button className="h-11 w-full rounded-lg bg-ember font-black text-white disabled:opacity-60" disabled={busy}>{t(language, "submit")}</button>
      </form>

      {listings.length === 0 ? <p className="py-12 text-center text-sm text-ink/60">{t(language, "empty")}</p> : listings.map((listing) => (
        <article key={listing.id} className="rounded-lg border border-black/10 bg-white p-3">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h3 className="font-black">{listing.title}</h3>
              <p className="text-sm text-ink/60">{listing.status} · expires {new Date(listing.expires_at).toLocaleDateString()}</p>
            </div>
            <button className="grid h-10 w-10 place-items-center rounded-full bg-red-50 text-red-600" onClick={() => onDelete(listing)} aria-label="Delete">
              <Trash2 size={18} />
            </button>
          </div>
        </article>
...
function AdminQueue({ busy, payments, onReview }: { busy: boolean; payments: PaymentRequest[]; onReview: (id: string, action: "approve" | "reject") => void }) {
  return (
    <section className="space-y-3 p-4">
      {payments.length === 0 ? <p className="py-12 text-center text-sm text-ink/60">No pending payments</p> : payments.map((payment) => (
        <article key={payment.id} className="space-y-3 rounded-lg border border-black/10 bg-white p-3">
          <div>
            <h2 className="font-black">{payment.upgrade_type} · {payment.amount_etb} ETB</h2>
            <p className="text-sm text-ink/60">{payment.listings?.title ?? "Extra post slot"}</p>
          </div>
          <a className="block rounded-md bg-mist p-3 text-sm underline" href={payment.screenshot_url ?? getStoragePublicUrl("payment-screenshots", payment.screenshot_path)} target="_blank">
            Open receipt screenshot
          </a>
          <div className="grid grid-cols-2 gap-2">
            <button className="h-11 rounded-lg bg-leaf font-black text-white disabled:opacity-60" disabled={busy} onClick={() => onReview(payment.id, "approve")}>Approve</button>
            <button className="h-11 rounded-lg bg-red-600 font-black text-white disabled:opacity-60" disabled={busy} onClick={() => onReview(payment.id, "reject")}>Reject</button>
          </div>
        </article>
      ))}
    </section>
...
function ToastView({ toast, onClose }: { toast: Toast; onClose: () => void }) {
  ...
  return (
    <button className={`fixed left-4 right-4 top-4 z-50 mx-auto max-w-[448px] rounded-lg p-3 text-sm font-bold text-white ${toast.type === "success" ? "bg-leaf" : "bg-red-600"}`} onClick={onClose}>
      {toast.message}
    </button>
  );
}
...
const sampleListings: Listing[] = [
  ...
  },
  {
    ...
    type: "job",
    title: "Cafe cashier",
    description: "Full time cashier needed. Basic Amharic reading and friendly service required.",
    price: null,
    salary: 6000,
